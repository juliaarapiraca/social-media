...
const comentar = async (req, res) => {
    ...
    if (!texto) {
        return res.status(404).json('Para comentar nessa postagem é preciso informar o texto.');
    }
...
const feed = async (req, res) => {
    ...
    try {
        ...
        for (const postagem of postagens) {
            ...
            postagem.curtidoPorMim = curtida.find(curtida => curtida.usuario_id === id) ? true : false;

            const comentarios = await knex('postagem_comentarios').leftJoin('usuarios', 'usuarios.id', 'postagem_comentarios.usuario_id').where({ postagem_id: postagem.id }).select('usuarios.username', 'postagem_comentarios.texto');
